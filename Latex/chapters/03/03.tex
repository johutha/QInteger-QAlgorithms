...
Die im obigen Produkt vorkommenden Zustände nennen wir in diesem Zusammenhang die \textit{Fourierbasis}\footnote{Man beachte, dass die Fourierbasis keine Vektorraumbasis im Sinno von Kapitel 2.1.2 ist.}.
...
\begin{minipage}{.33\textwidth}
    \centering
    \begin{blochsphere}[radius=0.5\textwidth, ball=circle]
        \drawBall
    \end{blochsphere}
\end{minipage}
\begin{minipage}{.33\textwidth}
...
\begin{enumerate}
    ...
    \item Für jedes $j \in \{0, ..., n - 1\}$, wende für jedes Bit $x_k$ mit $k \leq j$ ein kontrolliertes $Rot(j + 1 - k)$ auf das Qubit im $y$-Register im Zustand $\frac{1}{\sqrt{2}}\left(\ket{0} + e^{2i\pi\frac{y}{1 + j}}\right)$ an. Dieses befindet sich danach im Zustand:
    \begin{align*}
